...
app.post("/api/generate", async (req, res) => {
  ...
  if (!input) {
    return res.status(400).json({
      error: { code: "EMPTY_INPUT", message: "Please enter a topic or some notes." }
    });
  }

  if (input.length > 8000) {
    return res.status(400).json({
      error: { code: "INPUT_TOO_LONG", message: "Please keep the input under 8,000 characters." }
    });
  }
  ...
  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).json({
      error: {
        ...
        message: "GEMINI_API_KEY is missing. Add it to .env and restart the server."
      }
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(502).json({
        error: {
          code: "PROVIDER_ERROR",
          message: payload?.error?.message || `The AI provider returned HTTP ${response.status}.`
        }
      });
    }
    ...
    if (!raw) {
      return res.status(502).json({
        error: { code: "EMPTY_MODEL_RESPONSE", message: "The AI returned an empty response." }
      });
    }
    ...
    if (error?.name === "AbortError") {
      return res.status(504).json({
        error: { code: "TIMEOUT", message: "The AI request took too long. Please try again." }
      });
    }

    console.error("Generation error:", error);
    return res.status(500).json({
      error: { code: "SERVER_ERROR", message: "Something went wrong while generating the deck." }
    });
  } finally {
...
app.listen(port, () => {
  console.log(`StudySpark backend running on http://localhost:${port}`);
});